...
static cl::OptionCategory CrochetDiffCategory("crochet-diff options");

static cl::opt<bool>
    ASTDump("ast-dump",
            cl::desc("Print the internal representation of the AST."),
            cl::init(false), cl::cat(CrochetDiffCategory));
...
static cl::opt<std::string> TargetPath("target",  cl::desc("<target>"), cl::cat(CrochetDiffCategory));

static cl::opt<std::string> SourcePath(cl::Positional, cl::desc("<source>"),
                                       cl::Required,
                                       cl::cat(CrochetDiffCategory));
...
static cl::opt<int> MaxSize("s", cl::desc("<maxsize>"), cl::Optional,
                            cl::init(-1), cl::cat(CrochetDiffCategory));

static cl::opt<float> MinSimilarity("min-sim", cl::desc("<minsimilarity>"), cl::Optional,
                            cl::init(-1), cl::cat(CrochetDiffCategory));

static cl::opt<std::string> BuildPath("p", cl::desc("Build path"), cl::init(""),
                                      cl::Optional, cl::cat(CrochetDiffCategory));
...
static void printNodeAttributes(raw_ostream &OS, diff::SyntaxTree &Tree,
  ...
  std::vector<std::string> exceptions {"TranslationUnitDecl", "OpaqueValueExpr", "NullStmt"};
  const diff::Node &N = Tree.getNode(Id);
  OS << R"("id":)" << int(Id);  
  OS << R"(,"type":")" << N.getTypeLabel() << '"';
  auto Offsets = Tree.getSourceRangeOffsets(N);
  if (!in_array(N.getTypeLabel(), exceptions)){
    auto StartLoc = Tree.getSourceBeginLocation(N);   
    auto EndLoc = Tree.getSourceEndLocation(N);   
...
static void printDstChange(raw_ostream &OS, diff::ASTDiff &Diff,
  ...
  {
    OS << "Default (" << DstNode.Change << ") ";
    printNode(OS, SrcTree, Src);    
    //std::string Value = DstTree.getNodeValue(Dst);
    //replaceSubString(Value, "\n", "");
...
int main(int argc, const char **argv) {
  ...
  cl::HideUnrelatedOptions(CrochetDiffCategory);
